# Hi, I'm Valeria 👋

3rd-year Computer Engineering student at Kryvyi Rih National University.  
I build desktop applications with C# and .NET — focused on clean architecture,  
multithreading, and real-world usability. Looking for my first junior developer role.

---

## 🛠 Tech Stack

![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet&logoColor=white)
![WPF](https://img.shields.io/badge/WPF-0078D4?style=flat&logo=windows&logoColor=white)
![XAML](https://img.shields.io/badge/XAML-0C54C2?style=flat&logo=xaml&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)

---

## 🔨 Featured Project

### [MultiThreaded Study Planner](https://github.com/ValeriaKarpachova/MultiThreadedStudyPlanner)
A desktop app for managing student workload — task scheduling, deadline analysis,  
automatic priority calculation, Pomodoro timer, and background monitoring.  
Built with WPF · .NET 10 · SQLite · LiveChartsCore · multithreading.

---

## 📚 Currently Learning

- **Entity Framework Core** — ORM for .NET, natural next step after raw SQLite
- **MVVM pattern** — the standard architecture for WPF/desktop apps
- **ASP.NET Core basics** — to eventually build APIs alongside desktop clients

---

## 📫 Contact

[![Telegram](https://img.shields.io/badge/Telegram-26A5E4?style=flat&logo=telegram&logoColor=white)](https://t.me/akana_san)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/valeria-karpachova-3bb0ab385)
